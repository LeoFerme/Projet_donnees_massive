...
def parse_args():
    ...
    p.add_argument('--cleanup', action='store_true', help="Nettoie User et Post avant de seeder.")
    return p.parse_args()
...
def create_posts(client: datastore.Client, names: list[str], total_posts: int, dry: bool):
    ...
    created = 0
    batch_size = 500  # Limite de 500 entités par appel à put_multi
    posts_batch = []
    ...
    for i in range(total_posts):
        ...
        if len(posts_batch) >= batch_size or i == total_posts - 1:
            if not dry and posts_batch:
                # Écriture par lot (beaucoup plus rapide !)
                client.put_multi(posts_batch)
                created += len(posts_batch)
                posts_batch = [] 
...
def cleanup_datastore(client: datastore.Client):
    """Supprime toutes les entités User et Post en utilisant la pagination."""
    ...
    for kind in kinds_to_clean:
        ...
        iterator = query.fetch(limit=BATCH_SIZE)
        
        for page in iterator.pages:
            keys_to_delete = [entity.key for entity in page]
            num_keys_in_batch = len(keys_to_delete)
            
            if num_keys_in_batch == 0:
                break 
                
            client.delete_multi(keys_to_delete)
            total_deleted += num_keys_in_batch

        if total_deleted > 0:
            print(f"Supprimé {total_deleted} entités de type '{kind}'.")
        else:
